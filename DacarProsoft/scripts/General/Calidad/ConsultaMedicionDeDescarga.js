...
function GenerarPdf() {
    $("#ModalConfirmacionTablaRegistros").modal("show");
}

$('#BtnConfirmacionTablaRegistros').on("click", function (e) {
    e.stopPropagation();
    $("#ModalConfirmacionTablaRegistros").modal("hide");
    var canvas = document.getElementById('myChart');
    var dataURL = canvas.toDataURL();
    SetViewBag(dataURL);

    var url = "../Calidad/GenerarPdfReporteAutodescarga?idMeidicionDescarga=" + idAutodes +"&variable=Si";
    window.open(url);
});

$('#BtnNegacionTablaRegistros').on("click", function (e) {
    e.stopPropagation();
    $("#ModalConfirmacionTablaRegistros").modal("hide");
    var canvas = document.getElementById('myChart');
    var dataURL = canvas.toDataURL();
    ...
    var url = "../Calidad/GenerarPdfReporteAutodescarga?idMeidicionDescarga=" + idAutodes + "&variable=No";
    window.open(url);
});

$('#BtnConfirmacionTablaRegistros2').on("click", function (e) {
    e.stopPropagation();
    variable = "Si";
    $("#ModalConfirmacionTablaRegistros2").modal("hide");
...
$('#BtnNegacionTablaRegistros2').on("click", function (e) {
    e.stopPropagation();
    variable = "No";
    $("#ModalConfirmacionTablaRegistros2").modal("hide");
...
function AbrirModalEnvio() {
    $("#ModalConfirmacionTablaRegistros2").modal("show");
    //var canvas = document.getElementById('myChart');
    //var dataURL = canvas.toDataURL();
    //SetViewBag(dataURL);
...
function EnviarPdf() {
    ...
    if ($("#txtCorreoDestino").val() == "" || $("#txtCorreoCopia").val() == "") {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { idMeidicionDescarga: idAutodes, Correo: $("#txtCorreoDestino").val(), CorreoCopia: $("#txtCorreoCopia").val(), variable: variable },
            success: function (msg) {
                $('#BtnEnvio').prop('disabled', false);
                $("#BtnEnvio").text("Enviar");
        ...
        success: function (msg) {
            ...
            $("#tblDetalleMedicionesDescarga").dxDataGrid({
                ...
                columns: [
                    ...
                    {
                        dataField: "Voltaje", caption: "Voltaje", alignment: "left", allowHeaderFiltering: false, allowSearch: true,
                        format: {
                            type: "fixedPoint",
                            precision: 2,
                        },
